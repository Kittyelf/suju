...
Menu.prototype.eventBind = function() {
	var t = this;
	var lastCenter = util.cookie.get("lastCenter");
	if(lastCenter != null && lastCenter!=""){
		this.select($("li[url='"+lastCenter+"']",this.menuList));
		this.onClick({
			"url":lastCenter
		});
	}else{
	...
	$(t.menuList).on("mouseenter",".data-frame-menu",function(event){
		...
		t.select(this);
		util.cookie.set("lastCenter",$(this).attr("url"));
		t.onClick({
			"url":$(this).attr("url")
		});
